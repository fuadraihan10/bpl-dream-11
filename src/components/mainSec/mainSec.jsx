...
function MainSec({ selectedPlayers, setSelectedPlayers }) {
    ...
    <div className="max-w-screen-2xl mx-auto">
        <div className="flex justify-between items-center p-4 ">
            ...
            <div>
                ...
                <button id="selected" onClick={selectedFunc} className={`md:text-xl text-sm px-2 py-1 rounded-r-xl border border-l-0 border-gray-200 md:px-4 md:py-2 ${activeId === "selected" ? "bg-yellow-300" : ""}`}>Selected (0)</button>
            </div>

        </div>
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 ${activeId === "selected" ? "hidden" : ""}`}>
            {players.map((player, index) => (
                <Player key={index} player={player} selectedPlayers={selectedPlayers} setSelectedPlayers={setSelectedPlayers} />
            ))}
        </div>
        <div>
            {
